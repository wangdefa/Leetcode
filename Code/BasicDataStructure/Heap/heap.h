...
class Heap {
 ...
 public:
  ...
  // Floyd constructor
  Heap(typename std::vector<T>::iterator first,
...
// Floyd constructor
template <class T>
Heap<T>::Heap(typename std::vector<T>::iterator first,
              typename std::vector<T>::iterator last,
              std::function<bool(T, T)> cmp_func) {
  cmp = cmp_func;
  ...
  for (; index != -1; index--) {
    ...
    while (true) {
      ...
      if (this->GetLeftChild(index_copy) != -1 &&
          cmp(heap[this->GetLeftChild(index_copy)], heap[valid_child])) {
        valid_child = this->GetLeftChild(index_copy);
      }
      if (this->GetRightChild(index_copy) != -1 &&
          cmp(heap[this->GetRightChild(index_copy)], heap[valid_child])) {
...
void Heap<T>::Insert(T data) {
  ...
  while (this->GetParent(index) != -1 &&
         cmp(heap[index], heap[this->GetParent(index)])) {
    T tmp = heap[index];
...
bool Heap<T>::Delete() {
  ...
  while (true) {
    ...
    if (this->GetLeftChild(index) != -1 &&
        cmp(heap[this->GetLeftChild(index)], heap[valid_child])) {
      valid_child = this->GetLeftChild(index);
    }
    if (this->GetRightChild(index) != -1 &&
        cmp(heap[this->GetRightChild(index)], heap[valid_child])) {
